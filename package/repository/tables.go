...
func (repo *TablePostDB) UpdateTable(Id int, post model.TablePost, logrus *logrus.Logger) (int64, error) {
	tm := time.Now()
	query := fmt.Sprintf("	UPDATE %s SET post_title =$1, post_img_url  = $2, post_body = $3,  updated_at=$4 WHERE id = $5 RETURNING id", tables)
...
func (repo *TablePostDB) GetTableById(id string, logrus *logrus.Logger) (model.TableFull, error) {
	...
	query := fmt.Sprintf("SELECT  id, post_title, post_img_path,post_img_url, post_body, post_date  FROM %s WHERE id=$1 ", tables)
