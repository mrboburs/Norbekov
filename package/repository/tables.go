...
func (repo *TablePostDB) GetAllTable(logrus *logrus.Logger) (array []model.TableFull, err error) {
	rowsRs, err := repo.db.Query("SELECT id,post_title,post_img_path,post_img_url, post_body, post_date,price,duration ,post_title_ru,post_body_ru FROM tables")
	...
	for rowsRs.Next() {
		snb := model.TableFull{}
		err = rowsRs.Scan(&snb.ID, &snb.PostTitle, &snb.PostImgPath, &snb.PostImgUrl, &snb.PostBody, &snb.PostDate, &snb.Price, &snb.Duration, &snb.PostTitleRu, &snb.PostBodyRu)
		if err != nil {
			logrus.Infof("ERROR: not scanning data from sql %s", err.Error())
			// log.Println(err)
			// http.Error(w, http.StatusText(500), 500)
...
func (repo *TablePostDB) CreateTablePost(post model.TablePost, logrus *logrus.Logger) (int, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (post_title ,post_img_url,   post_body ,price,duration,post_title_ru,post_body_ru) VALUES ($1, $2, $3,$4,$5,$6,$7)  RETURNING id", tables)

	row := repo.db.QueryRow(query, post.PostTitle, post.PostImgUrl, post.PostBody, post.Price, post.Duration, post.PostTitleRu, post.PostBodyRu)
...
func (repo *TablePostDB) UpdateTable(Id int, post model.TablePost, logrus *logrus.Logger) (int64, error) {
	tm := time.Now()
	query := fmt.Sprintf("	UPDATE %s SET post_title =$1, post_img_url  = $2, post_body = $3,  updated_at=$4,price=$5,duration=$6,post_title_ru=$7,post_body_ru=$8  WHERE id = $9 RETURNING id", tables)
	rows, err := repo.db.Exec(query, post.PostTitle, post.PostImgUrl, post.PostBody, tm, post.Price, post.Duration, post.PostTitleRu, post.PostBodyRu, Id)
...
func (repo *TablePostDB) GetTableById(id string, logrus *logrus.Logger) (model.TableFull, error) {
	...
	query := fmt.Sprintf("SELECT  id, post_title, post_img_path,post_img_url, post_body, post_date,price,duration ,post_title_ru,post_body_ru FROM %s WHERE id=$1 ", tables)
